<?php
class ModuleReplaceAction extends ModuleAction 
{
    ...
    public function replaceType(){
        ...
        $this->data['templateBtn'] = '<a href="'. C('LtUrl')->generate('Module', 'Replace', array('name'=>$this->data['name'], 'do'=>'directReplace')).'"> <button type="button" class="btn btn-success btn-lg">确认修改</button></a>
   ';
    ...
    public function directReplace(){
        ...
        //执行更新远程文件
        $info = $this->getLibObject("ShellService")->doShell($this->data['name'], 'find_file_replace', '-A');
        $msg = '';
        $out = '';
        if($info['status']===0){
            $msg = '内容修改操作执行完成！';
