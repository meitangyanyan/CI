<?php
class ModuleGrayAction extends ModuleAction 
{
    ...
    public function grayType(){

        $this->template = strtolower($this->context->uri["module"]) . "-confirmupdate";  //这行必须有
        $this->data['templateBtn'] = '<a href="'. C('LtUrl')->generate('Module', 'Gray', array('name'=>$this->data['name'], 'do'=>'directGray')).'"> <button type="button" class="btn btn-success btn-lg">确认灰度发版</button></a>
   ';

    }

    public function directGray(){

        $this->template = strtolower($this->context->uri["module"]) . "-confirmupdate"; //这行必须有
        $msg = '';

        //执行更新远程文件
        $info = $this->getLibObject("ShellService")->doShell($this->data['name'], 'gray_update', '-A');
        $msg = '';
        $out = '';
        if($info['status']===0){
            $msg = '模块更新操作执行完成！';
