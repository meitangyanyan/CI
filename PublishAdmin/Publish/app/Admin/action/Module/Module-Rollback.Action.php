<?php
class ModuleRollbackAction extends ModuleAction 
{
	...
	public function execute() {
        ...
        //取得版本列表
        error_reporting(1);  //以下日志级别是error
        $content = file_get_contents("https://docker.gaoxiaobang.com/api/repositories/tags?repo_name=prod%2F".$this->data['name']);
        if (!$content){
            $this->data['module_version'] = null;
        }else{
            $obj = json_decode($content);
            rsort($obj);
            $this->data['module_version'] = $obj;
        }
