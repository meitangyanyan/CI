<?php
class ModuleRestartAction extends ModuleAction 
{
    ...
    public function restartType(){

        $this->template = strtolower($this->context->uri["module"]) . "-confirmupdate";  //这行必须有
        $this->data['templateBtn'] = '<a href="'. C('LtUrl')->generate('Module', 'Restart', array('name'=>$this->data['name'], 'do'=>'directRestart')).'"> <button type="button" class="btn btn-success btn-lg">确认重启</button></a>
   ';

    }

    public function directRestart(){

        $this->template = strtolower($this->context->uri["module"]) . "-confirmupdate"; //这行必须有
        $msg = '';

        //执行更新远程文件
        $info = $this->getLibObject("ShellService")->doShell($this->data['name'], 'restart', '-A');
        $msg = '';
        $out = '';
        if($info['status']===0){
            $msg = '模块更新操作执行完成！';
